...
import { EXAM_CONFIG } from "@/lib/ap-human-geography/config";

describe("AP Human Geography Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM183");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("ap-human-geography");
  });
