...
import { emitPrismSignal } from "@/lib/ap-human-geography/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "ap-human-geography" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "ap-human-geography" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "ap-human-geography" });
